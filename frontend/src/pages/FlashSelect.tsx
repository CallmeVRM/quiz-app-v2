...
export default function FlashSelect() {
      ...
      <ContentCard title="Catégories" style={{ marginBottom: "1.5rem" }}>
        <MultiSelect
          options={cats.map(c => ({
            ...
            badge: c.counts ? `${c.counts.flashcards ?? 0} F` : undefined
          }))}
          selected={selCats}
          onChange={setSelCats}
        ...
        {selCats.length > 0 ? (
          <MultiSelect
            options={subs.map(su => {
              ...
              const f = su.sub.counts?.flashcards ?? 0;
              return {
                value: token,
